...
class Table:
    def __init__(self, config, chatlog, log):
        ...
        self.col_flags = [
            ...
            True,  
            True,  
            bool(config.table.get("skin", True)),  
            True,  
            bool(config.table.get("rr", True)),  
            bool(config.table.get("peakrank", True)),  
    ...
    def set_columns(self):
        ...
        for field in self.fields_to_display:
            self.rich_table.add_column(t(COLUMN_LABEL_KEYS[field]), justify="center")
