...
NESTED_PRESENCE_KEYS = ("matchPresenceData", "partyPresenceData", "playerPresenceData", "premierPresenceData")

def flatten_presence(presence_dict):
    for nested_key in NESTED_PRESENCE_KEYS:
        nested = presence_dict.get(nested_key)
        if isinstance(nested, dict):
            presence_dict.update(nested)
        ...
        for presence in presences:
            if presence['puuid'] == self.Requests.puuid:
                if presence.get("championId") is not None or presence.get("product") == "league_of_legends":
                    ...
                    return flatten_presence(json.loads(base64.b64decode(presence['private'])))

    def decode_presence(self, private):
        if "{" not in str(private) and private is not None and str(private) != "":
            dict = flatten_presence(json.loads(base64.b64decode(str(private)).decode("utf-8")))
            if dict.get("isValid"):
                return dict
        return {
            "isValid": False,
