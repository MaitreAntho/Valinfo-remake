...
import re
from src.presences import flatten_presence
        ...
        if len(m) > 10:
            ...
            if resp_json[2].get("uri") == "/chat/v4/presences":
                ...
                if presence['puuid'] == self.Requests.puuid:
                    if presence.get("championId") is not None or presence.get("product") == "league_of_legends":
                        state = None
                        decoded_presence = None
                    else:
                        try:
                            decoded_presence = flatten_presence(json.loads(base64.b64decode(presence['private'])))
                            state = decoded_presence.get("sessionLoopState")
                        except json.JSONDecodeError:
                            # presence blob is occasionally empty during a state transition; just skip this tick
                            decoded_presence = None
                            state = None

                    if state is not None:
                        if self.cfg.get_feature_flag("discord_rpc"):
                            self.rpc.set_rpc(decoded_presence)
            ...
            if resp_json[2].get("uri") == "/chat/v6/messages":
                try:
                    ...
                    self.chatlog(f"failed to handle chat message: {e}")
